...
- **Gameplay Flow**: Pieces spawn, move, rotate, fall, lock, clear lines, send garbage lines to the opponent, score, gradually speed up during the match, detect loss, continue after one player loses, show winner/draw, and restart.
- **Special Objects**: Every 4 seconds, each active board can spawn a shuffled-bag special object on a supported free tile with a clear approach path. Objects expire after 10 seconds, timed effects last 10 seconds, speed effects are 2x, `R`/`r` add a 2-second lag to rotation inputs for 10 seconds, and the teleporter swaps boards/pieces without changing scores.
